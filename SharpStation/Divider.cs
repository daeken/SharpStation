namespace SharpStation {
	public static class Divider {
		public static uint Divide(ushort numerator, ushort divisor) {
			var shift = divisor.CountLeadingZeros();
			
			var n = (ulong) numerator << shift;
			var d = (ushort) (divisor << shift);
			
			var reciprocal = (ulong) Reciprocal(d);

			var res = (n * reciprocal + 0x8000) >> 16;
			return res <= 0x1FFFF ? (uint) res : 0x1FFFF;
		}

		static readonly byte[] UnrTable = {
		...
		static uint Reciprocal(ushort d) {
			var factor = UnrTable[((d & 0x7fff) + 0x40) >> 7] + 0x101;
			var di = d | 0x8000;
			var tmp = (di * -factor + 0x80) >> 8;
			return (uint) ((factor * (0x20000 + tmp) + 0x80) >> 8);
		}
	}
}